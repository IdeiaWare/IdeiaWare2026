...
public class HearQuestionControllerTest {
	...
	public void mostrarPergunta_semCookie_redireciona() throws Exception {
		mvc.perform(get("/persona/empatia/o-que-escuta").param("personaId", "1"))
				.andExpect(view().name("redirect"));
	}

	@Test
	public void mostrarPergunta_comCookie_mostraView() throws Exception {
		when(service.getAttributes(1, "hear", 5L)).thenReturn(java.util.Collections.<Empathy>emptyList());

		mvc.perform(get("/persona/empatia/o-que-escuta")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("personaId", "1"))
				.andExpect(status().isOk())
				.andExpect(view().name("hear"));
	}
	...
	public void deletar_comCookieEEtapaCF_deleta() throws Exception {
		mvc.perform(post("/persona/empatia/o-que-escuta/delete")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("personaId", "1")
				.param("attributeId", "9"))
				.andExpect(status().is3xxRedirection());
