...
// TEST-02: SeeQuestionController -- valida o backstop server-side (TK-VAL) e o TK-ATTR
// (atributo "see" forcado no servidor) via MockMvc.
public class SeeQuestionControllerTest {
	...
	@Before
	public void setup() {
		SeeQuestionController controller = new SeeQuestionController();
	...
	public void mostrarPergunta_semCookie_redireciona() throws Exception {
		mvc.perform(get("/persona/empatia/o-que-ve").param("personaId", "1"))
				.andExpect(view().name("redirect"));
	}

	@Test
	public void mostrarPergunta_comCookie_mostraView() throws Exception {
		when(service.getAttributes(1, "see", 5L)).thenReturn(java.util.Collections.<Empathy>emptyList());

		mvc.perform(get("/persona/empatia/o-que-ve")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("personaId", "1"))
				.andExpect(status().isOk())
				.andExpect(view().name("see"));
	}
	...
	public void ideiaForaDaEtapaCF_naoSalva() throws Exception { // UX-TOOLKIT-STATUS-GUARD
		...
		mvc.perform(post("/persona/empatia/o-que-ve/save-attribute")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("attributeText", "Ve anuncios no feed")
				.param("personaId", "1"))
	...
	public void deletar_comCookieEEtapaCF_deleta() throws Exception {
		mvc.perform(post("/persona/empatia/o-que-ve/delete")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("personaId", "1")
				.param("attributeId", "9"))
				.andExpect(status().is3xxRedirection());
	...
	public void attributeForcadoNoServidor_ignoraValorDoCliente() throws Exception { // TK-ATTR
		...
		org.junit.Assert.assertEquals("see", captor.getValue().getAttribute());
	}
}
