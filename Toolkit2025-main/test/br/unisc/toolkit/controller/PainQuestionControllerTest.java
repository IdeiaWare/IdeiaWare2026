...
// TEST-02: PainQuestionController -- valida o backstop server-side (TK-VAL) e o TK-ATTR
// (atributo "pain" forcado no servidor) via MockMvc.
public class PainQuestionControllerTest {
	...
	@Before
	public void setup() {
		PainQuestionController controller = new PainQuestionController();
	...
	public void mostrarPergunta_semCookie_redireciona() throws Exception {
		mvc.perform(get("/persona/empatia/quais-sao-as-dores").param("personaId", "1"))
				.andExpect(view().name("redirect"));
	}

	@Test
	public void mostrarPergunta_comCookie_mostraView() throws Exception {
		when(service.getAttributes(1, "pain", 5L)).thenReturn(java.util.Collections.<Empathy>emptyList());

		mvc.perform(get("/persona/empatia/quais-sao-as-dores")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("personaId", "1"))
				.andExpect(status().isOk())
				.andExpect(view().name("pain"));
	}
	...
	public void ideiaForaDaEtapaCF_naoSalva() throws Exception { // UX-TOOLKIT-STATUS-GUARD
		...
		mvc.perform(post("/persona/empatia/quais-sao-as-dores/save-attribute")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("attributeText", "Medo de falhar")
				.param("personaId", "1"))
	...
	public void deletar_comCookieEEtapaCF_deleta() throws Exception {
		mvc.perform(post("/persona/empatia/quais-sao-as-dores/delete")
				.cookie(new Cookie("ideiaId", "5"), new Cookie("ideiaSig", AssinaturaCaixa.assinar("5")))
				.param("personaId", "1")
				.param("attributeId", "9"))
				.andExpect(status().is3xxRedirection());
	...
	public void attributeForcadoNoServidor_ignoraValorDoCliente() throws Exception { // TK-ATTR
		...
		org.junit.Assert.assertEquals("pain", captor.getValue().getAttribute());
	}
}
