...
public class HearQuestionController {
	...
	public String showThinkAndFeel(@RequestParam("personaId") int theId, Model theModel, HttpServletRequest request){
		...
		if(cookie.getCookieIdeiaCodigo(request) != null){
			...
			List<Empathy> theEmpaties = empathyService.getAttributes(theId, "hear", ideiaCodigo);
			
			// create model attribute to bind form data
			Empathy theEmpathy = new Empathy();
					
			theModel.addAttribute("pageTitle", "O que Escuta");
	...
	@PostMapping("/o-que-escuta/save-attribute")
	public String saveAttribute(@ModelAttribute("attribute") Empathy theEmpathy, BindingResult result, HttpServletRequest request){
		if(cookie.getCookieIdeiaCodigo(request) != null){
		   ...
		   return "redirect:/persona/empatia/o-que-escuta?personaId=" + theEmpathy.getPersonaId();
		}
		else{
			return "redirect";
		}
	...
	@GetMapping("/o-que-escuta/delete")
								...
								Model theModel, HttpServletRequest request)
	{
		...
		return "redirect:/persona/empatia/o-que-escuta?personaId=" + personaId;
	}
}
