...
public class RetornaAudioTest {
	...
	private Ideia novaIdeia(Usuario autor) {
		Ideia ideia = new Ideia(autor, "Ideia RetornaAudio", "desc", StatusIdeia.STORYTELLING, StatusIdeia.GRUPO_ABERTO);
		ideia.setDtCriacao();
	...
	public void semStorytellingIdNaSessao_retorna400() throws Exception {
		...
		new RetornaAudio().doGet(request, response);

		verify(response).setStatus(HttpServletResponse.SC_BAD_REQUEST);
	}

	@Test
	public void semAudioSalvo_retornaListaVazia() throws Exception {
	...
	@Test
	public void comAudioSalvo_retornaSomenteElementosDeAudio() throws Exception {
		Usuario autor = novoUsuario("Autor2");
		Ideia ideia = novaIdeia(autor);
		Storytelling st = novoStorytelling(autor, ideia);

		ElementosStorytelling audio = new ElementosStorytelling(st, "AUD", "base64-audio-data", 0, 0, 0, 0);
		elementosStorytellingDAO.salvar(audio);
		ElementosStorytelling imagem = new ElementosStorytelling(st, "IMG", "", 0, 0, 10, 10);
		elementosStorytellingDAO.salvar(imagem);
		...
		assertEquals("base64-audio-data", lista.get(0).getAsJsonObject().get("caminho").getAsString());
	}
}
