...
public class AbrirPointOfViewTest {
	...
	private Ideia novaIdeia(Usuario autor) {
		Ideia ideia = new Ideia(autor, "Ideia POV", "desc", StatusIdeia.CAIXA_FERRAMENTAS, StatusIdeia.GRUPO_ABERTO);
		ideia.setDtCriacao();
	...
	private HttpServletRequest mockRequest(Long codigoUsuario, String codigoPOV) {
		...
		when(request.getParameter("codigoPOV")).thenReturn(codigoPOV);
		return request;
	}
	...
	public void semLogin_retorna401() throws Exception {
		...
		new AbrirPointOfView().doGet(request, response);
