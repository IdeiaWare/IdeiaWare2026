...
<body>
  <%-- UX-COLAB-BOTOES-HEADER: "Finalizar Colaboração"/"Editar Texto" subiram do corpo pro cabecalho. --%>
  <c:if test="${sessionScope.lider eq 'S' and sessionScope.isRetencao eq false}">
    <form id="editarTextoForm" action="EditarTextoServlet" method="post" style="display:none;">
      <input type="hidden" value="${sessionScope.ideiaId}" name="ideiaId">
    </form>
    <form id="finalizarColabForm" action="FinalizarColaboracaoServlet" method="post" style="display:none;">
      <input type="hidden" value="${sessionScope.ideiaId}" name="ideiaId">
    </form>
  </c:if>
  <nav>
    <div class="nav-wrapper teal darken-2 z-depth-2">
      ...
      <%-- UX-COLAB-HEADER-LIMPO: itens de admin escondidos so' nesta tela (ficavam 4+ itens juntos). --%>
      <ul id="nav-mobile" class="right hide-on-med-and-down">
        <c:if test="${sessionScope.lider eq 'S' and sessionScope.isRetencao eq false}">
          <li><a href="javascript:;" onclick="document.getElementById('editarTextoForm').submit();"><i class="material-icons left">edit</i>Editar Texto</a></li>
          <li><a href="javascript:;" onclick="if (confirm('Finalizar a ideia encerra a colaboração. Esta ação não pode ser desfeita. Confirmar?')) document.getElementById('finalizarColabForm').submit();"><i class="material-icons left">flag</i>Finalizar Colaboração</a></li>
        </c:if>
